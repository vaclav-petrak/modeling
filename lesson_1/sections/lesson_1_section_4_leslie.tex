...
\textbf{Your Task:}
\begin{enumerate}
    \item Construct the Leslie Matrix $L$ in Matlab.
...
\begin{lstlisting}
years = 20;
% Pre-allocate matrix for results (3 rows, years+1 columns)
pop_history = zeros(3, years + 1);
...
\begin{enumerate}
    ...
    \item Plot the specific age classes over time (use \lstinline{hold on}).
\end{enumerate}
\end{frame}
...
\tiny
\begin{lstlisting}
% Calculate total population (sum of each column)
total_pop = sum(pop_history);
...
figure;
subplot(2,1,1);
plot(t, total_pop, 'k-', 'LineWidth', 2);
title('Total Population Growth');
xlabel('Years'); ylabel('Count');
grid on;

subplot(2,1,2);
plot(t, pop_history(1,:), 'b-o', 'DisplayName', 'Chicks'); hold on;
plot(t, pop_history(2,:), 'r-o', 'DisplayName', 'Young Adults');
plot(t, pop_history(3,:), 'g-o', 'DisplayName', 'Old Adults');
legend('show');
...
proportions = pop_history ./ sum(pop_history);

% Display the proportions for the last year
disp('Final Age Distribution:');
disp(proportions(:, end));

% Check the previous year to see if it stabilized
disp('Previous Year Distribution:');
disp(proportions(:, end-1));
\end{lstlisting}
